...
|---|---|---|---|
|No database or empty SQLite file|Create schema version 4 transactionally|Creates the canonical tables, FTS5/vocabulary projections, PDF metadata columns, triggers, and `schema_meta` marker|Index explicitly selected sources|
|Schema version 4 with the expected shape|Open without changing canonical rows|Validate required tables/columns; rebuild FTS5 only when its row count is inconsistent|Reopen, then re-index approved roots if source files are available|
|Schema version 3 with the expected shape|Run the reviewed v3→v4 transaction|Preserve hashes, extractor identity/version, anchors, relationship rows, and source identity; add PDF page/warning metadata with safe defaults|Reopen and re-index approved roots|
|Schema version 2 with the expected shape|Run the reviewed v2→v4 transaction|Preserve hashes, extractor identity/version, anchors, relationship rows, and source identity; add `index_jobs` and PDF metadata with safe defaults|Reopen and re-index approved roots|
...
- The checked-in migration fixture is [`tests/fixtures/schema-v2.sql`](../tests/fixtures/schema-v2.sql).
  ...
  marker.
- Opening a version-2 fixture creates `index_jobs`, PDF metadata defaults, and records version 4 in
  one transaction. The migration never recomputes or replaces canonical hashes, extractor identity,
  anchors, or relationships.
...
- A malformed version-2 marker fails with a named reason such as `schema version 2 is missing
  ...
  remain untouched.
- Version-3 databases receive `parse_warnings_json` (`[]`) and nullable `page_count` columns in a
...
Schema version 4 is the supported local format. Versions 2 and 3 are supported by reviewed
